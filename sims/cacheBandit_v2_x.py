...
NOTES:  -> Up switch and deallocator are merged into one module, called the refresh unit.
        -> The refresh Unit is triggered each time the thread changes pages. CDC is through
...
contexts = 4                # No. of thread contexts TDM'd by worker @ Freq_cacheBandit
...
class bandit:
    ...
    def __init__ (self, block_id, page_size, pages, ipc=1):
        ...
        page_sol_init_val = lambda x: self.page_size-self.reclaim_size if (x == 0) else self.page_size
        self.cur_sol = [page_sol_init_val(x) for x in range(self.pages)]
        self.cur_page = 0
        self.next_page = 0
    ...
    def dealloc_refresh (self, page):
        ...
        vptr = beg if (self.init) else random.randint(beg, end)
        this_page = page
        # Copy page context
...
class thread:
    ...
    def __init__ (self, thread_id, virtual_line, virtual_assoc, virtual_sets, page_size, pages, ipc=1):
        # Thread
        self.tid = thread_id
            ...
            if (t.tid != tid and found == 0):
                ...
                for line_id, line in enumerate(s):
                    if (line[0] == key and line[3]):
                        #print (f'{thread.find_L2.__name__} [{tid}] ||| Found key [{key}] in L2 at location [{set_sel}][{line_id}], in thread [{t.tid}] | line = {line}', flush=True)
                        found = 1
                        ptr = line[1][0]
                        page = line[1][1]
                        # Function only cares about finding in L2. 
                        # Hardware must make sure owner (src) does not over-write line
                        # while cloner (dst) is copying that line.
                        cls.__L2_hit += 1
                        break
    ...
    def print_stats (self):
        ...
        print (f'{thread.print_stats.__name__} ||| ------ THREAD[{self.tid}] STATS BEGIN ------ ')
        print (f'{thread.print_stats.__name__} ||| Total accesses = {total_access} | mem_ext_hit = {self.ext_hit}, mem_int_hit = {self.int_hit} | hit_rate = {hit_rate}, miss_rate = {miss_rate}')
        print (f'{thread.print_stats.__name__} ||| ---------- THREAD END ----------')
    ...
    def invalidate (self, alloc, set_sel, line, vld):
        ...
        if (alloc and vld):
            #print (f'{thread.invalidate.__name__} ||| Invalidating set[{set_sel}], line[{line}] = {self.vcache[set_sel][line]}', flush=True)
    ...
    def find_lru (self, set_sel, vset):
        ...
        for line_id, line in enumerate(vset):
            ...
            #print (f'{thread.find_lru.__name__} [{self.tid}] ||| LRU || Frame = {line[0]}, phy_ptr = {line[1]}, line_life = {line_life}, base_life = {base_life}', flush=True)
        lru_meta = min(comparator)
        lru_id = lru_meta[1]
        self.vlru[set_sel] = lru_id
        #print (f'{thread.find_lru.__name__} [{self.tid}] ||| LRU || list = {self.vlru}')
        ...
        for sid, s in enumerate(self.vcache):
            for lid, line in enumerate(s):
                if (line[2] > 0):
                    line[2] -= 1
                    #if (line[2] < 0):
                    #    print (f'{thread.age_cache.__name__} ||| Age turns negative!')
                    #    sys.exit(1)
    
    def show_cache(self, set_sel):
        print (f'{thread.show_cache.__name__} [{self.tid}] ||| Dumping Cache state:\n')
        for set_id, s in enumerate(self.vcache):
            if (set_id == set_sel or set_sel == self.vsets):
                for line_id, line in enumerate(s):
                    print (f'{thread.show_cache.__name__} [{self.tid}] ||| Set[{set_id}] || Line[{line_id}] = {line}', flush=True)
    
    def find_buf (self, key):
        found = 0
        if (self.buf[0] == key):
            #print (f'{thread.find_buf.__name__} [{self.tid}] ||| Found key [{key}] in Buffer', flush=True)
            found = 1
        return found
    ...
    def find_L1 (self, set_sel, key):
        ...
        for line_id, line in enumerate(s):
            if (line[0] == key and line[3]):
                #print (f'{thread.find_L1.__name__} [{self.tid}] ||| Found key [{key}] in L1 at location [{set_sel}][{line_id}] | line = {line}', flush=True)
                found_L1 = 1
                at = line_id
                ptr = line[1][0]
                page = line[1][1]
        ...
        #print (f'{thread.find_L1.__name__} [{self.tid}] ||| found = {found}, at = {at}, ptr = {ptr}, page = {page}, life = {life}, vld = {vld} | alloc = {alloc}', flush=True)
        ...
        if (found_L2):
        ...
        elif (found_L1 == 0 and found_L2 == 0):
            if (vld):
                alloc = 0
            ...
            #print (f'{thread.find_L1.__name__} [{self.tid}] ||| Inserting key [{key}] in L1 at location [{set_sel}][{ev_lid}] | evicted_line = {evict_line}, inserted_line = {[key, (ptr, page), life, vld]} | alloc = {alloc}', flush=True)
    ...
    def strm (self, taddr):
        ...
        #print (f'{thread.strm.__name__} ||| ------- RUN Thread [{self.tid}] ------- || strm_addr = {taddr} | Key = {vframe} | Set = {vset_sel}', flush=True)
...
def workers_test (ctxt_fname, vline, vassoc, vsets, page_size, pages, ipc):
    ...
    worker = list(thread(tid, vline, vassoc, vsets, page_size, pages, ipc) for tid in range(workers))
